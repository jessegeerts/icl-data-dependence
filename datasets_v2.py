...
def generate_input_seqs(mus_label, mus_class, labels_class, S, N, Nmax, eps=0.1, B=0, p_B=0, P=None, p_C=0,
                        flip_labels=False, output_target_labels=False, no_repeats=False):
    e_fac = 1 / np.sqrt(1 + eps ** 2)
    ...
    (e_fac * (mus_class[choices] + eps * np.random.normal(size=(S, N, D)) / np.sqrt(D)))[filt_C]
    ...
    ((e_fac * (mus_class[targets] + eps * np.random.normal(size=(S, D)) / np.sqrt(D))))[filt_C]
    # JPG: for each ~filt_C we fill in new few shot seqs?
    inputs[~filt_C, :-1:2, 2 * Nmax + 1:] = \
    (e_fac * (mus_class_new[choices_c] + eps * np.random.normal(size=(S, N, D)) / np.sqrt(D)))[~filt_C]
    inputs[~filt_C, 1:-1:2, 2 * Nmax + 1:] = ((mus_label[labels_class_new])[choices_c])[~filt_C]
    inputs[~filt_C, -1, 2 * Nmax + 1:] = \
    (e_fac * (mus_class_new[targets_c] + eps * np.random.normal(size=(S, D)) / np.sqrt(D)))[~filt_C]
